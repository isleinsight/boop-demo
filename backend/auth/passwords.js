...
// unified password helpers (uses your existing file)
const { hashPassword, verifyPassword } = require('../passwords');

// ── Email (Postmark) ─────────────────────────────────────────────────────────
const { ServerClient } = require('postmark');

// Where should the *link in emails* send people?
// You can override with RESET_BASE_URL in .env (default is APP_URL, defaulting to https://payulot.com)
const APP_URL = (process.env.APP_URL || 'https://payulot.com').replace(/\/+$/, '');
const RESET_BASE_URL = (process.env.RESET_BASE_URL || APP_URL).replace(/\/+$/, '');

// Postmark creds/sender
const POSTMARK_TOKEN = process.env.POSTMARK_SERVER_TOKEN || '';
const FROM_EMAIL = process.env.SENDER_EMAIL || 'no-reply@payulot.com';
const postmark = POSTMARK_TOKEN ? new ServerClient(POSTMARK_TOKEN) : null;

// Optional override for the logo URL in the email. Defaults to Payulot logo under /assets/logo.png
const EMAIL_LOGO_URL = process.env.EMAIL_LOGO_URL || `${RESET_BASE_URL}/assets/logo.png`;
...
console.log('✅ password.js router loaded');

// ── Helpers ──────────────────────────────────────────────────────────────────
function generateToken() { return crypto.randomBytes(32).toString('hex'); }
function hashToken(raw) { return crypto.createHash('sha256').update(raw).digest('hex'); }
function isValidPassword(pw) { return typeof pw === 'string' && pw.length >= PASSWORD_MIN_LEN; }
      ...
      <tr>
        <td align="center" style="padding:24px;">
          <table width="640" cellpadding="0" cellspacing="0" role="presentation" style="max-width:640px;width:100%;background:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 8px 28px rgba(16,24,40,.08);">
            <tr>
              <td style="background:#0f243f; padding:20px 24px;" align="left">
                <img src="${EMAIL_LOGO_URL}" width="140" height="auto" alt="Payulot" style="display:block;border:0;outline:none;">
              </td>
            </tr>
            <tr>
...
router.post('/forgot-password', async (req, res) => {
  ...
  res.json({ ok: true }); // anti-enumeration
...
router.post('/reset', async (req, res) => {
  ...
  if (!isValidPassword(pw)) return res.status(400).json({ error: `Password must be at least ${PASSWORD_MIN_LEN} characters.` });
  ...
  try {
    ...
    await db.query('DELETE FROM sessions WHERE user_id=$1', [userId]); // force sign-out
...
router.post('/change-password', authenticateToken, async (req, res) => {
  ...
  if (!isValidPassword(new_password)) return res.status(400).json({ error: `Password must be at least ${PASSWORD_MIN_LEN} characters.` });

...
const db = require('../../db');                  // ✅ correct path to backend/db.js
const { authenticateToken } = require('../middleware/authMiddleware');

// ✅ unified password helpers (argon2 or bcryptjs based on availability / ENV)
...
/**
 * URL bases:
 * - RESET_LINK_BASE: where the button/link should send users to set password
 *   (defaults to APP_URL, finally to https://payulot.com)
 * - ASSET_BASE: where to load images from in the email (defaults to RESET_LINK_BASE)
 *   e.g. https://payulot.com
 */
const APP_URL = (process.env.APP_URL || 'https://payulot.com').replace(/\/+$/, '');
const RESET_LINK_BASE = (process.env.RESET_LINK_BASE || APP_URL).replace(/\/+$/, '');
const ASSET_BASE = (process.env.ASSET_BASE || RESET_LINK_BASE).replace(/\/+$/, '');

const POSTMARK_TOKEN = process.env.POSTMARK_SERVER_TOKEN || '';
const FROM_EMAIL =
  process.env.SENDER_EMAIL ||
  `no-reply@${new URL(RESET_LINK_BASE).hostname}`;
...
/** Build absolute link to password page, always including mode=reset */
function buildResetLink(rawToken) {
  return `${RESET_LINK_BASE}/password.html?token=${encodeURIComponent(rawToken)}&mode=reset`;
}

/** Use Payulot branding and a dark header so white logo pops */
function buildEmailHTML({ title, intro, ctaText, ctaUrl, footerNote }) {
  const logoUrl = `${ASSET_BASE}/assets/logo.png`; // <- your Payulot logo filename
      ...
      <tr>
        <td align="center" style="padding:24px;">
          <table width="640" cellpadding="0" cellspacing="0" role="presentation" style="max-width:640px; width:100%; background:#ffffff; border-radius:14px; overflow:hidden; box-shadow:0 8px 28px rgba(16,24,40,.08);">
            <tr>
              <td style="background:#0f1f38; padding:20px 24px;" align="left">
                <img src="${logoUrl}" width="140" height="auto" alt="${brand}" style="display:block; border:0; outline:none;">
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td style="padding:16px 24px; background:#f9fafb; color:#6b7280; font-size:12px;">
                Sent by ${brand} • <a href="${RESET_LINK_BASE}" style="color:#6b7280; text-decoration:none;">${new URL(RESET_LINK_BASE).hostname}</a>
              </td>
            </tr>
          </table>
        </td>
...
// Pretty wrappers with distinct subjects & copy (Payulot)
...
router.post('/admin/initiate-reset', authenticateToken, async (req, res) => {
  try {
    ...
    // Email (admin flavor) → Payulot password page
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    // Email (forgot flavor) → Payulot password page
    const link = buildResetLink(raw);

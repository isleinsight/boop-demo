...
function passportId() {
  // <= 64 chars, easy to read
  return 'PID-' + randHex(12);
}
function pidToken() {
  return 'PTK-' + randHex(24);
}
function cardUID() {
  return 'CARD-' + randHex(12);
}
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const dupe = await client.query('SELECT id FROM users WHERE lower(email) = lower($1)', [cleanEmail]);
    if (dupe.rowCount) {
    ...
    // 3) create user (type=cardholder, status=active)
    const userIns = await client.query(
      `
      INSERT INTO users (email, first_name, middle_name, last_name, password_hash, type, status)
      VALUES ($1, $2, $3, $4, $5, 'cardholder', 'active')
      RETURNING id
      `,
    ...
    await client.query(`UPDATE users SET wallet_id = $1 WHERE id = $2`, [walletId, userId]);
    ...
    await client.query(`UPDATE users SET passport_pid = $1 WHERE id = $2`, [ppid, userId]);
    ...
    // Handle common unique errors cleanly
    const msg = /unique/i.test(String(e?.message)) ? 'A unique constraint failed (email, passport, or card UID).' : 'Failed to create account.';

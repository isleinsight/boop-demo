...
// PATCH /api/users/:id — update user (name, email, role, status, parent_id)
router.patch("/:id", async (req, res) => {
  ...
  try {
    await pool.query(`UPDATE users SET ${updates.join(", ")} WHERE id = $${values.length + 1}`, [...values, id]);
...
// GET /api/users?parentId=XYZ — get assigned students
...
// (Optional) POST /api/users/:id/signout — force signout (stub only)
router.post("/:id/signout", async (req, res) => {
  // Implement session/token invalidation logic here

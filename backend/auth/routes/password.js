...
// ✅ FIX: correct path to DB pool
const db = require('../../db.js');

// ✅ shared password helpers (argon2 or bcryptjs depending on env/availability)
...
// Where should reset/setup links point? (public site)
const RESET_BASE_URL =
  (process.env.RESET_BASE_URL || process.env.APP_URL || 'https://payulot.com')
    .replace(/\/+$/, '');

// Where should the logo image be loaded from?
const ASSET_BASE =
  (process.env.ASSET_BASE || 'https://payulot.com').replace(/\/+$/, '');

// Postmark credentials + From:
const POSTMARK_TOKEN = process.env.POSTMARK_SERVER_TOKEN || '';
const FROM_EMAIL =
  process.env.SENDER_EMAIL ||
  `no-reply@${new URL(RESET_BASE_URL).hostname}`;
...
// Email HTML (Payulot branding)
function buildEmailHTML({ title, intro, ctaText, ctaUrl, footerNote }) {
  const logoUrl = `${ASSET_BASE}/assets/logo.png`; // <- Payulot logo path
      ...
      <tr>
        <td align="center" style="padding:24px;">
          <table width="640" cellpadding="0" cellspacing="0" role="presentation" style="max-width:640px;width:100%;background:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 8px 28px rgba(16,24,40,.08);">
            <tr>
              <td style="background:#0f1e33;padding:20px 24px;" align="left">
...
// Email variants (wording aligned to Payulot)
...
router.post('/admin/initiate-reset', authenticateToken, async (req, res) => {
  try {
    ...
    // Email (admin flavor) — always route to Payulot password page with ?mode=reset
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    // Email (forgot flavor) — send to Payulot password page with ?mode=forgot

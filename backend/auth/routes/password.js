...
<body>
  ...
  <footer>
    <div class="footer-inner">
      <div>© <span id="year"></span> BOOP</div>
  ...
  <script>
    (async function(){
      ...
      try {
        ...
        const res = await fetch("/api/me", { headers: { Authorization: `Bearer ${token0}` }});
      ...
      ownerNameEl.textContent = `${first}${last ? ' ' + last : ''}`;
      ...
      function cardTileHTML(card){
        ...
        const title = card.type === 'assistance' ? 'Assistance Card' : 'Spending Card';
        const ending = card.last4 ? `Card ending in ${card.last4}` : null;
        return `
          <div class="tile">
            <div class="row">
              <h3>${title}</h3>
            ...
            ${ending ? `<div class="meta">${ending}</div>` : ``}
            <div class="meta">Use at supported merchants.</div>
            <div class="divider"></div>
            <div class="row">
              <div>
                <span class="label">Funding</span>
                <div class="value">BOOP Wallet</div>
              </div>
            </div>
          </div>
        `;
      ...
      function renderCards(cards){
        ...
        cardsList.innerHTML = cards.map(cardTileHTML).join('');
      }

      // ---------- Load passport + cards (using your existing routes) ----------
      async function loadPassportAndCards(){
        // A) Passport status/id (optional if you have /api/passport/mine)
        let passport = null;
        try{
        ...
        const pstatus = normalizeStatus(passport?.status || 'active');
        ...
        if (!walletId){ renderCards([]); return; }

        // C) Fetch cards by wallet_id
        try{
          const r = await fetch(`/api/cards?wallet_id=${encodeURIComponent(walletId)}`, {
            headers: { Authorization:`Bearer ${token}` }
          });
          const rows = await r.json();
          if (!Array.isArray(rows)){ renderCards([]); return; }

          const normalized = rows.map(c => ({
            type: normalizeType(c.type ?? c.card_type ?? c.kind),
            ...
            last4: pickLast4(c),
            owner_id: c.user_id ?? c.owner_id ?? null,
            passport_id: c.passport_id ?? null
          }));
      ...
      document.getElementById('buyTransitBtn').href = 'https://shorelink.example/'; // placeholder

 ...
 * Admin-only (accountant or treasury)
 */
...
/**
 * 📊 GET /api/transactions/report?start=YYYY-MM-DD&end=YYYY-MM-DD&type=credit|debit
 * Admin-only (accountant or treasury)
 */
...
router.get('/user/:userId', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Optional: server-side log to verify names
 ...
 * Admin-only (accountant or treasury)
 * Body: { treasury_wallet_id, wallet_id, user_id, amount, note }
 * - Debits the selected treasury wallet
 * - Credits the recipient wallet
 * - Uses sender_id (treasury owner) and recipient_id (target user)
 */
router.post('/add-funds', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // (Optional) Verify recipient wallet belongs to user_id
